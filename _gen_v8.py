...
def smart_crop_direct_xyz(
    ...
    if use_cuda:
        ...
        for x0 in x_coarse:
            for y0 in y_coarse:
                for z0 in z_coarse:
                    w = a_t[x0 : x0 + target_n, y0 : y0 + target_n, z0 : z0 + target_n].reshape(-1)
                    w = w[w != ig]
                    if w.numel() == 0:
                        dfrac = 1.0
                        uq = 0
                        ...
                        if mn < 0:
                            w = w - mn
                        counts = torch.bincount(w)
                        counts = counts[counts > 0]
                        uq = int(counts.numel())
                        dfrac = float(counts.max().item()) / float(total_vox)
                    coarse.append((dfrac, int(x0), int(y0), int(z0)))
    else:
        for x0 in x_coarse:
            for y0 in y_coarse:
                for z0 in z_coarse:
                    w_np = block_xyz[x0 : x0 + target_n, y0 : y0 + target_n, z0 : z0 + target_n]
                    dfrac, uq = _dominant_unique_numpy(w_np.reshape(-1), int(ignore_label), total_vox)
                    coarse.append((dfrac, int(x0), int(y0), int(z0)))
    ...
    if use_cuda:
        ...
        for x0, y0, z0 in refine:
            ...
            if best is None or key < best[0]:
                best = (key, x0, y0, z0, dominant_frac, unique_count)
    else:
        for x0, y0, z0 in refine:
            w_np = block_xyz[x0 : x0 + target_n, y0 : y0 + target_n, z0 : z0 + target_n]
    ...
    if best is None:
        ...
        _, x0, y0, z0, dominant_frac, unique_count = best

    cropped = block_xyz[x0 : x0 + target_n, y0 : y0 + target_n, z0 : z0 + target_n]
    best_info = (int(x0), int(y0), int(z0), 0.0, int(unique_count), float(dominant_frac), int(nx), int(ny), int(nz))
...
def add_fold_soft(h: NoddyHistory, rng: np.random.Generator, name: str, amp_range: Tuple[float, float] = (120.0, 420.0), wl_range: Tuple[float, float] = (1200.0, 2800.0), z_range: Tuple[float, float] = (-1800.0, -220.0), axis_tol: float = 20.0, plunge_range: Tuple[float, float] = (6.0, 35.0)) -> None:
...
def add_fault_soft(h: NoddyHistory, rng: np.random.Generator, name: str, slip_range: Tuple[float, float], dip_dir_tol: float = 12.0) -> None:
    dip_dir = sample_angle_avoid(rng, 0.0, 360.0, avoid=[0.0, 90.0, 180.0, 270.0], tol=dip_dir_tol)
    dip = float(rng.uniform(55.0, 85.0))
    # Mixture pitch strategy: mostly dip-slip-like for clearer vertical offsets,
    # with some strike-slip-like faults for diversity.
    if rng.random() < 0.65:
        pitch = float(rng.choice([-1.0, 1.0]) * rng.uniform(60.0, 88.0))
    else:
        pitch = float(rng.uniform(-30.0, 30.0))
    pz = float(rng.uniform(-1600.0, -180.0))
    slip = float(rng.uniform(float(slip_range[0]), float(slip_range[1])))
    px = float(rng.uniform(-1300.0, 1300.0))
    py = float(rng.uniform(-1300.0, 1300.0))
...
def add_unconformity_soft(h: NoddyHistory, rng: np.random.Generator, name: str, dip_dir_tol: float = 12.0, pz_range: Tuple[float, float] = (-280.0, -230.0), dip_range: Tuple[float, float] = (0.0, 6.0), n_layers_fixed: Optional[int] = None, n_layers_range: Tuple[int, int] = (1, 2), t_range: Tuple[int, int] = (1, 10), max_total_thickness: int = 10, target_n: int = 50) -> None:
        ...
        try:
            ...
            if (not explicit_folds) or (not explicit_faults) or (not explicit_unconf):
                ...
                if (not explicit_faults) and ("fault" in chosen):
                    add_fault_soft(h, rng, name="FAULT_1", slip_range=(180.0 * float(slip_scale_legacy), 520.0 * float(slip_scale_legacy)))
                if (not explicit_unconf) and ("unconf" in chosen):
                    should_add_unconf = True

            if should_add_unconf:
                if unconf_fixed_layers is not None:
                    add_unconformity_soft(h, rng, name="UNCONF_1", n_layers_fixed=int(unconf_fixed_layers), target_n=int(target_n))
                else:
                    add_unconformity_soft(h, rng, name="UNCONF_1", target_n=int(target_n))
            ...
            if use_torch and (torch_device is not None) and (torch_device.type == "cuda"):
                ...
                if vals.numel() == 0:
                    ...
                    mn = int(vals.min().item())
                    if mn < 0:
                        vals = vals - mn
                    counts = torch.bincount(vals)
                    counts = counts[counts > 0]
                    n_strat_present = int(counts.numel())
                ...
                if vals_np.size == 0:
                    ...
                    n_strat_present = int(counts_np.size)
