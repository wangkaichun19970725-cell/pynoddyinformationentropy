...
def smart_crop_direct_xyz(
    ...
    if use_cuda:
        ...
        for x0, y0, z0 in refine:
            ...
            key = (dominant_frac, -unique_count, -bsum)
...
def _estimate_unconf_reference_z(h: NoddyHistory, rng: np.random.Generator) -> float:
    ...
    pz = bottom_z + float(rng.uniform(0.35, 0.65)) * length_z
    eps = 1e-6
    pz = float(np.clip(pz, bottom_z + eps, top_z - eps))
    return pz


def _estimate_unconf_thicknesses(h: NoddyHistory, target_n: int) -> List[int]:
    _, _, length_z = h.get_extent()
    length_z = float(length_z)
    ...
    cropped_cap = int(max(2 * marker, np.floor(0.05 * (float(max(1, int(target_n))) * cube_size))))
    model_cap = int(max(2 * marker, np.floor(0.05 * length_z)))
    total_thickness = int(max(2 * marker, min(cropped_cap, model_cap)))

    t1 = marker
    t2 = max(marker, total_thickness - t1)
...
def add_unconformity_soft(h: NoddyHistory, rng: np.random.Generator, name: str, dip_dir_tol: float = 12.0, pz_range: Tuple[float, float] = (-280.0, -230.0), dip_range: Tuple[float, float] = (0.0, 6.0), n_layers_fixed: Optional[int] = None, n_layers_range: Tuple[int, int] = (1, 2), t_range: Tuple[int, int] = (1, 10), max_total_thickness: int = 10, target_n: int = 50) -> None:
    ...
    thicknesses = _estimate_unconf_thicknesses(h, int(target_n))
...
def worker_one(args_tuple):
    (
        ...
        q_min_nonzero_frac, q_max_dominant_frac,
        ...
        try:
            ...
            if use_torch and (torch_device is not None) and (torch_device.type == "cuda"):
                ...
                if vals_np.size == 0:
                    ...
                    unique = int(counts_np.size)
                    dominant_frac = float(np.max(counts_np)) / float(block_zyx.size)

            if unique < 2:
                raise RuntimeError(f"quality_reject unique={unique} < 2")
...
def main():
    ...
    print(f"[INFO] Quality gate strength={float(args.quality):.2f} => dominant_frac_max={eff_q['max_dominant_frac']:.6f} min_nonzero_frac={eff_q['min_nonzero_frac']:.6f} min_unique={int(eff_q['min_unique'])}")
    ...
    with ctx.Pool(processes=int(args.workers)) as pool:
        ...
        def submit_one(i: int):
            ...
            work = (
                ...
                float(eff_q["min_nonzero_frac"]), float(eff_q["max_dominant_frac"]),
