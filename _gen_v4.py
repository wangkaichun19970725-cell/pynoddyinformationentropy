...
def add_unconformity_soft(h: NoddyHistory, rng: np.random.Generator, name: str, dip_dir_tol: float = 12.0, pz_range: Tuple[float, float] = (-280.0, -230.0), dip_range: Tuple[float, float] = (0.0, 6.0), n_layers_fixed: Optional[int] = None, n_layers_range: Tuple[int, int] = (1, 2), t_range: Tuple[int, int] = (1, 10), max_total_thickness: int = 10) -> None:
    ...
    pz_raw = float(rng.uniform(*pz_range))
    cand_abs = pz_raw
    cand_rel = top_z + pz_raw

    inside_abs = (bottom_z <= cand_abs <= top_z)
    inside_rel = (bottom_z <= cand_rel <= top_z)

    use_rel = False
    if inside_rel and (not inside_abs):
        use_rel = True
    elif inside_rel and inside_abs:
        if max(pz_range) <= 0.0 and (min(pz_range) >= -1.2 * length_z):
            use_rel = True
    else:
        if max(pz_range) <= 0.0:
            use_rel = True

    pz = cand_rel if use_rel else cand_abs
    eps = 1e-6
    ...
    cap_float = max(1.0, 0.05 * length_z)
    ...
    distance_to_top = max(0.0, top_z - pz)
    min_sum_needed_i = int(max(0.0, np.ceil(distance_to_top - cap_float)))

    if per_layer_max <= 0:
        per_layer_max = 1
    min_layers_by_cap = int(np.ceil(max(1, min_sum_needed_i) / float(per_layer_max))) if min_sum_needed_i > 0 else 1
    if num_layers < min_layers_by_cap:
        num_layers = min_layers_by_cap
    ...
    target_total_i = int(max(base_target, min_sum_needed_i, tmin * num_layers))

    max_feasible = per_layer_max * num_layers
    if target_total_i > max_feasible:
        need_layers = int(np.ceil(target_total_i / float(per_layer_max)))
        if need_layers > num_layers:
            num_layers = need_layers
        max_feasible = per_layer_max * num_layers
        if target_total_i > max_feasible:
        ...
        try:
            ...
            if explicit_unconf:
                if int(unconf_layers) > 0:
                    add_unconformity_soft(h, rng, name="UNCONF_1", n_layers_fixed=int(unconf_layers))
